...
const transactionMatchesEvent = (transaction, event, dateKey) => {
    ...
    if (event.type === 'subscription') {
        ...
        if (linkedSubscriptionId && linkedSubscriptionId === String(event.sourceId || '').trim()) return true;
        if (event.meta?.matchedTransactionId && String(transaction?.id || '') === String(event.meta.matchedTransactionId)) return true;
...
const isRealized = (event, dateKey, sourceData = {}, today) => {
    if (event.status === 'completed' || event.status === 'paid' || event.meta?.occurrenceStatus === 'paid') return true;
